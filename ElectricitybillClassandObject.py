    ...
    if num_customers <= 0:
        ...
        with open("electricity_bills.txt", "w") as file:
            for i in range(num_customers):
                ...
                consumed_units = float(input(f"Enter the number of units consumed by {customer_name}: "))
               
                if consumed_units >= 0:
                    # Create an instance of the ElectricityBillCalculator class
                    bill_calculator = ElectricityBillCalculator(customer_name, consumed_units)
                    ...
                    print(f"Invalid input. Units consumed by {customer_name} should be a non-negative number.")
except ValueError:
